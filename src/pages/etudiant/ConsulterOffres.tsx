...
import { 
    Search, MapPin, Euro, ArrowLeft, Send, 
    Loader2, Calendar, Briefcase, Globe, Info, Clock 
} from 'lucide-react';
    ...
    if (selectedOffre) {
        return (
            <div className="max-w-6xl mx-auto space-y-6 animate-in fade-in duration-300 pb-10">
                <Button variant="ghost" onClick={() => setSelectedOffre(null)} className="text-white/50 hover:text-white mb-4">
                        ...
                        <CardContent className="p-6">
                            <form onSubmit={handleApply} className="space-y-6">
                                <div className="space-y-2">
                                    <label className="text-[10px] font-bold text-white/40 uppercase tracking-widest">Lien de votre CV (PDF)</label>
                                    <Input 
                                        required 
                                        value={cv} 
                                ...
                                <div className="space-y-2">
                                    <label className="text-[10px] font-bold text-white/40 uppercase tracking-widest">Pourquoi vous ? (Lettre de motivation)</label>
                                    <textarea 
                                        required 
                                        value={lettre} 
                                ...
                                <Button type="submit" disabled={isSubmitting} className="w-full bg-blue-600 hover:bg-blue-700 font-bold py-7 rounded-xl shadow-lg shadow-blue-600/20 transition-all">
                                    {isSubmitting ? <Loader2 className="animate-spin" /> : <><Send size={18} className="mr-2"/> Envoyer ma candidature</>}
                                </Button>
                            </form>
                        </CardContent>
                    </Card>
    ...
    return (
        <div className="space-y-8 max-w-6xl mx-auto">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 border-b border-white/5 pb-8">
                <div>
                    <h2 className="text-4xl font-black text-white tracking-tight">Opportunités</h2>
                    <p className="text-white/40 mt-1">Découvrez les offres validées par vos enseignants responsables.</p>
                </div>
            ...
            {isLoading ? (
                <div className="flex flex-col items-center justify-center py-32 gap-4">
                    ...
                    <p className="text-white/20 font-medium italic">Synchronisation avec le serveur...</p>
                </div>
            ) : offres.length === 0 ? (
                <div className="text-center py-32 border-2 border-dashed border-white/5 rounded-[40px] bg-white/[0.02]">
                    ...
                    <p className="text-white/30 text-xl font-medium italic">Aucune offre n'est disponible pour le moment.</p>
                </div>
            ) : (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pb-20">
                    {offres.filter((o: any) => o.titre.toLowerCase().includes(searchTerm.toLowerCase())).map((offre: any) => (
                        <Card key={offre.id_offre} className="bg-neutral-800/40 border-white/5 hover:border-blue-500/40 hover:bg-neutral-800/60 transition-all cursor-pointer group rounded-3xl overflow-hidden" onClick={() => setSelectedOffre(offre)}>
                            <CardContent className="p-8">
                                <div className="flex justify-between items-start mb-6">
                                    <div className="space-y-1">
                                        <h3 className="text-2xl font-bold text-white group-hover:text-blue-400 transition-colors">{offre.titre}</h3>
                                        <p className="text-blue-400/80 font-semibold text-lg">{offre.entreprise_nom}</p>
                                    </div>
                                ...
                                <div className="flex flex-wrap gap-x-8 gap-y-4 text-sm text-white/50 mb-6">
                                    <div className="flex items-center gap-2 font-medium"><MapPin size={16} className="text-red-400/50"/> {offre.adresse}</div>
                                    <div className="flex items-center gap-2 font-medium"><Euro size={16} className="text-green-400/50"/> {offre.remuneration}€</div>
                                </div>
